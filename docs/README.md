<p align="center">
  <img src="lib/public/icon-192.png" width="120" alt="Mosai Forge  symbol" /><br>
  <img src="lib/public/ujju" width="280" alt="Mosai Forge " />
</p>
...
Mosai Forge  runs on your machine and serves the workspace to your browser. For access outside your local network, use a private network such as Tailscale.
...
Mosai Forge  currently includes integrations for:
...
```text
Browser → Mosai Forge  daemon → selected coding-agent runtime → model provider
```

Mosai Forge  does not relay project traffic through a Clay-hosted cloud. Your code leaves the machine only when the selected runtime sends it to its model provider, just as it would when you use that CLI directly.
...
Building something with Mosai Forge ? Share it in [GitHub Discussions](https://github.com/chadbyte/clay/discussions).

## Disclaimer

Mosai Forge  is not affiliated with the providers of the coding agents it supports. Product names and trademarks belong to their respective owners. The software is provided "as is," and users are responsible for complying with the terms of their selected providers.
